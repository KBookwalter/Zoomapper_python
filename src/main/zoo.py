...
class ZooMapper(tk.Tk):
    ...
    def popup_sheet_selection(self, sheet_names):
    	...
    	label = ttk.Label(popup, text='Please select a sheet name containing your data, then reopen this file.')
    	...
    	B2 = Button(popup, text = "Submit", command = lambda: self.submit_sheet_choice(popup, sheet_names, cl.get()))
    ...
    def get_plot_creation_options(self, data_frame, saved_import=None):
        ...
        while validParameters == False:
            try:
                ...
                #	break
                heat_options_box = HeatMapOptionsBox(StartPage, data_frame, (plot_options, 'heatmap_options'), self.spreadsheet_path, self.sheet_choice, saved_defaults=saved_import)
                ...
                if options['x_column'] == "":
                    ...
                    noName = messagebox.askyesno(title=None, message="Are you sure you do not want to choose a Name Column?", )
                    if noName:
            ...
            try:
                ...
                if file_type == ".xlsx":
                    ...
                    if len(xl_data.sheet_names) > 1:
                    	...
                    	if saved_import == None:
                    		popup = tk.Toplevel(self)
                    		choice_window = Sheet_Entry(self, popup, xl_data.sheet_names)
                    		choice_window.dr.wait_window(choice_window.dr)
                    		
                    	if self.sheet_choice != "":
                    		data = pd.read_excel(filename, sheet_name=self.sheet_choice)
                ...
                if self.selected_sheet and self.sheet_choice != "":
                	...
                	heatmap_options = self.get_plot_creation_options(data, saved_import=saved_import)
                	self.df = data
    ...
    def update_graph(self, sheet = None, saved_import=None):
        ...
        """
        if sheet != None:
            data = sheet.get_sheet_data(return_copy = False, get_header = False, get_index = False)
            col = sheet.get_sheet_data(return_copy=True, get_header = True, get_index = True)
            cols = []
            #getting columns
            for i in col:
            ...
            #print(newdata)
            heat_frame = HeatMapPage(self.container_please, self, newdata, heatmap_options)
            heat_frame.config(bg=LIGHT_BLUE)
            heat_frame.grid(row=0, column=0, sticky="nsew")
            self.frames[HeatMapPage] = heat_frame
        else:
    ...
    def calc_name_distance(self,parent,data_frame,options,cal_ratio):
        """
        ...
        if hasattr(self,"name_window"):
            ...
            self.name_window = CalculationInputPage(parent,data_frame,options,cal_ratio)
            self.name_window.wait_window(self.name_window)
            
            
class Sheet_Entry:

	def __init__(self, parent, owner, sheet_names):
	
		self.main_class = parent
	
		self.master = owner
		self.label = ttk.Label(self.master, text='Please select a sheet name containing your data, then reopen this file.')
		...
		self.dr.pack()
		self.B1 = Button(self.master, text = "Cancel", command = self.master.destroy)
		self.B1.pack(side="bottom", fill="y")
		self.B2 = Button(self.master, text = "Submit", command = lambda: self.submit_sheet_choice(self.master, sheet_names, self.cl.get()))
		self.B2.pack(side="bottom", fill="y")
		
	def submit_sheet_choice(self, parent, sheets, choice):
...
class LoadWindow(tk.Toplevel):
    ...
    def __init__(self, parent, out_dict):
        ...
        self.protocol("WM_DELETE_WINDOW", lambda arg=out_dict: self.return_cancel(arg))
        ...
        b_import['command'] = lambda: self.return_file_to_import(saves, out_dict, saved_imports)
    ...
    def return_file_to_import(self, listbox, out_dict, saved_imports):
        """
...
class CalculationInputPage(tk.Toplevel):
    ...
    """    
    def __init__(self, parent,data_frame,options,cal_ratio):
        ...
        time_filter_list = HeatMapOptionsBox.filterOptions(self,[['Date'],['Time']], self.columns_list)

        if "Date" in time_filter_list and "Time" in time_filter_list:
            time_filter_list.append('Date + Time')

        self.time_column_dropdown = tk.OptionMenu(frm, self.time_column_var, *time_filter_list)
        ...
        def check_date_time(*args):
            ...
            if self.time_column_var.get() in ["Date", "Time"]:
                self.time_warning_label.config(text="Choosing "+str(self.time_column_var.get())+" will calculate only one distance per "+str(self.time_column_var.get())+".")
            else:
                self.time_warning_label.config(text = "")

        self.time_column_var.trace("w", check_date_time)

        self.time_warning_label = tk.Label(frm,text="", bg = "white", fg = "red")
    ...
    def save_file(self):
        ...
        outFile = filedialog.asksaveasfile(mode='w', filetypes=csv_save, defaultextension=csv_save)
        if outFile != None:
    ...
    def calculateDistances(self,data_frame,time_column,names):
        """
        ...
        """
        errCode = []
        if not time_column: errCode.append("A")
        if len(names) < 2: errCode.append("B")
        if not self.outputFile: errCode.append("C")
        if len(errCode) > 0:
            errMsg = ''
            if 'A' in errCode: errMsg = errMsg + 'Time Element is not specified.\n'
            if 'B' in errCode: errMsg = errMsg + 'Not enough names are specified (minimum 2 required).\n'
            if 'C' in errCode: errMsg = errMsg + 'No output file specified.\n'
            self.attributes('-topmost', 'false')
            self.wait_window(InputError(self, errMsg.rstrip('\n')))
            self.attributes('-topmost', 'true')
        else:
            thread = threading.Thread(target=self.calculateDistances_thread, args=(data_frame,time_column,names,))
            thread.start()
            #Destroys window when complete
            self.destroy()
            delattr(self.parent,"name_window")

    def calculateDistances_thread(self,data_frame,time_column,names):
        """
        Performs distance calculations and outputs to file.
        """
        # Process strings

        names = [HeatMapPage.standardize_string(name) for name in HeatMapPage.process_string_input(names)]
        ...
        for ind in range(len(names)):
            for ind2 in range(ind+1,len(names)):
                outputList.append("Distance from " + names[ind] + " to " + names[ind2])
        ...
        if(self.zcol_name!=''):
            for timestamp, time_data in split_data:
                if type(timestamp) == tuple:
                    if time_column == "Date" and len(str(timestamp))>10:
                        outputList.append(str(timestamp[0])[:10])
                    elif time_column == "Time" and len(str(timestamp))>10:
                        outputList.append(str(timestamp[1]))
                    else:
                        outputList.append(str(timestamp[0])[:10]+" "+str(timestamp[1]))
                else:
                    if time_column == "Date" and len(str(timestamp))>10:
                ...
                for row_index, row in time_data.iterrows():
                    name = HeatMapPage.standardize_string(row[self.name_column])
                    if name in names:
                        iterNames.append(name)
                        points.append((row[self.xcol_name], row[self.ycol_name],row[self.zcol_name]))
                            ...
                            for ind2 in range(ind+1,len(names)):
                                if names[ind2] in iterNames:
                                    outputList.append(self.calculateDistance2Names3D(
                                        points[iterNames.index(names[ind])][0],points[iterNames.index(names[ind])][1], points[iterNames.index(names[ind])][2],
                                        points[iterNames.index(names[ind2])][0],points[iterNames.index(names[ind2])][1],points[iterNames.index(names[ind2])][2]))
                                else:
            ...
            for timestamp, time_data in split_data:
                ...
                for row_index, row in time_data.iterrows():
                    ...
                    if name in names:
                        iterNames.append(name)
                        points.append((row[self.xcol_name], row[self.ycol_name]))
                ...
                if len(points) < 2:
                    ...
                    for ind in range(len(names)):
                        if names[ind] in iterNames:
                            for ind2 in range(ind+1,len(names)):
                                if names[ind2] in iterNames:
                                    outputList.append(self.calculateDistance2Names(
                                        points[iterNames.index(names[ind])][0],points[iterNames.index(names[ind])][1],
                                        points[iterNames.index(names[ind2])][0],points[iterNames.index(names[ind2])][1]))
                                else:
                                    outputList.append("NA")
                        else:
                            for ind2 in range(ind+1,len(names)):
    ...
    def calculateDistance2Names(self,x1,y1,x2,y2):
        """
        Performs 2D distance calculations using calibration ratios
        """
        if(self.x_ratio!=''and self.y_ratio!=''):
            return np.sqrt(np.square(x1*self.x_ratio-x2*self.x_ratio)+np.square(y1*self.y_ratio-y2*self.y_ratio))
        else:
            return np.sqrt(np.square(x1-x2)+np.square(y1-y2))*self.cal_ratio
    
    def calculateDistance2Names3D(self,x1,y1,z1,x2,y2,z2):
        """
        Performs 3D distance calculations using calibration ratios
        """
        if(self.x_ratio!=''and self.y_ratio!='' and self.z_ratio!=''):
            return np.sqrt(np.square(x1*self.x_ratio-x2*self.x_ratio)+np.square(y1*self.y_ratio-y2*self.y_ratio)+np.square(z1*self.z_ratio-z2*self.z_ratio))
        else:
            return np.sqrt(np.square(x1-x2)+np.square(y1-y2)+np.square(z1-z2))*self.cal_ratio
...
class CalculationsCompleteMessage(tk.Toplevel):
    ...
    def __init__(self, filename):
        ...
        self.enter_text = tk.Label(frm, text="The calcuations for " + filename + " have finished")
...
class SheetView(tk.Tk, tk.Toplevel):
    ...
    """
    def __init__(self, dataframe, options, controller):
        tk.Tk.__init__(self)
        menu=tk.Menu(self)
        self.config(menu=menu)
        self.original_df = dataframe
        subMenu=tk.Menu(menu, tearoff=0)
        subMenu.add_command(label="Save New Spreadsheet", command = lambda : self.save_current_spreadsheet())
        subMenu.add_command(label="Sort Sheet Data By...", command = lambda : self.handle_sort_spreadsheet(dataframe))
        subMenu.add_command(label="Filter Sheet Data By...", command = lambda : self.handle_filter_selection(dataframe, options))
        subMenu.add_command(label="Remove Filter...", command = lambda : self.handle_remove_filter(dataframe, options))
        ...
        self.protocol("WM_DELETE_WINDOW", lambda: self.close(controller))
        ...
        self.sheet = Sheet(self.frame,
                           data = dftemp.values.tolist(), headers = list(dataframe.columns),
                           width=1440, height=810, column_width=200)
        self.sheet.enable_bindings()
        self.sheet.popup_menu_add_command("Update plot", self.update_plot)
    ...
    def remove_filter(self, dataf, ops):
    	...
    	self.removeFilterMenu.protocol("WM_DELETE_WINDOW", lambda: self.filter_del_return())
    	self.removeFilterMenu.wm_title("Choose a Filter Name to Remove")
    	currentLabel = tk.Label(self.removeFilterMenu, text="Current Filters:", font=("Helvetica", 10))
    	currentLabel.pack(side="top", fill="x", pady=10)
    	for filter_name in ops['filters']:
    		filterLabel = tk.Label(self.removeFilterMenu, text="{} -> {}".format(filter_name, ops['filters'][filter_name]), font=("Helvetica", 8))
    		filterLabel.pack(side="top", fill="x", pady=10)
    	label1 = tk.Label(self.removeFilterMenu, text="Please choose a filter name", font=("Helvetica", 10))
    	...
    	if len(options) != 0:
    		...
    		drop.pack()
    		button1 = tk.Button(self.removeFilterMenu, text="Submit", command = lambda : self.submit_remove_choice(tkvar, ops))
    		button1.pack()
    		button2 = tk.Button(self.removeFilterMenu, text="Cancel", command = self.filter_del_return)
    		button2.pack()
    ...
    def filter_spreadsheet(self, df, opts):
        ...
        self.filterMenu.protocol("WM_DELETE_WINDOW", lambda: self.filter_return())
        self.filterMenu.wm_title("Choose a Column and Value To Filter By")
        currentLabel = tk.Label(self.filterMenu, text="Current Filters:", font=("Helvetica", 10))
        currentLabel.pack(side="top", fill="x", pady=10)
        
        for filter_name in opts['filters']:
        	filterLabel = tk.Label(self.filterMenu, text="{} -> {}".format(filter_name, opts['filters'][filter_name]), font=("Helvetica", 8))
        	filterLabel.pack(side="top", fill="x", pady=10)
        	
        label1 = tk.Label(self.filterMenu, text="Please choose a column name", font=("Helvetica", 10))
        label1.pack(side="top", fill="x", pady=10)
        ...
        label2 = tk.Label(self.filterMenu, text="Please enter a value to filter by", font=("Helvetica", 10))
        label2.pack(side="top", fill="x", pady=10)
        
        data_val_box = tk.Text(self.filterMenu, height=1, width = 32)
        data_val_box.pack(side="top", pady=10)
        
        button1 = tk.Button(self.filterMenu, text="Submit", command = lambda : self.submit_filter_choice(tkvar, data_val_box, opts))
        button1.pack()
        button2 = tk.Button(self.filterMenu, text="Cancel", command = self.filter_return)
        button2.pack()
    ...
    def submit_remove_choice(self, cvar, opts):
    	...
    	for k in opts['filters']:
    		if opts['filters'][k].isnumeric():
    			...
    			dataframe = dataframe[dataframe[k]==str(opts['filters'][k])]
    	
    	self.current_df = dataframe
    	self.update_sheet(self.current_df)
    	self.removeFilterMenu.destroy();delattr(self, 'removeFilterMenu')
    ...
    def submit_filter_choice(self, cvar, textbox, ops):
        """
        ...
        if dataframe[column_choice].dtypes == 'object':
            self.current_df = dataframe.loc[(dataframe[column_choice].astype(str).apply(HeatMapPage.standardize_string, 1).isin(allowed))]
        else:
            self.current_df = dataframe.loc[(dataframe[column_choice].isin(allowed))]
        
        # Update GUI and variables
        ops['filters'][column_choice] = ', '.join([str(val) for val in allowed])
        self.update_sheet(self.current_df)
        self.filterMenu.destroy();delattr(self, 'filterMenu')
    ...
    def submit_sort_choice(self, svar):
        ...
        if objtype == float or objtype == int:
            self.current_df[svar.get()] = pd.to_numeric(self.current_df[svar.get()], errors = 'coerce')
        elif objtype == pd.Timestamp:
            self.current_df[svar.get()] = pd.to_datetime(self.current_df[svar.get()], errors = 'coerce')
    ...
    def sort_spreadsheet(self, df):
        ...
        """
        self.sortMenu=tk.Toplevel(self);self.sortMenu.protocol("WM_DELETE_WINDOW", lambda: self.sort_return())
        self.sortMenu.wm_title("Choose a Column To Sort By")
        label = tk.Label(self.sortMenu, text="Please choose a column name from the list below", font=("Helvetica", 10))
        label.pack(side="top", fill="x", pady=10)
        ...
        button1 = tk.Button(self.sortMenu, text="Submit", command = lambda : self.submit_sort_choice(tkvar))
        button1.pack()

        button2 = tk.Button(self.sortMenu, text="Cancel", command = self.sort_return)
        button2.pack()
    ...
    def save_current_spreadsheet(self):
        ...
        """
        types = [('CSV file (.csv)', '*.csv'), ('Excel file (.xlsx)', '*.xlsx')]
        file = filedialog.asksaveasfile(filetypes=types, defaultextension="*.csv", initialfile=".csv")

        if file:
            path,ext = os.path.splitext(file.name)

            if ext == '.csv':
                self.current_df.to_csv(path_or_buf=file.name, index=False)
            else:
                self.current_df.to_excel(excel_writer=file.name, index=False)	
